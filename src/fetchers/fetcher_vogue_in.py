...
# This is the library that can fetch a "feed" from a Vogue.in page
#
# example: http://www.vogue.in/fashion/fashion-insider/
...
def process_toi(source_info, options, debug):
    ...
    if debug:
        print(u"Processing Vogue_in fetcher for:")
        pprint.pprint(source_info)
        print(u"Fetching Vogue_in from: {}".format(html_source).encode("utf-8"))
...
# now lets parse the html
    ...
    # They make it easy "articles" are inside the <article> tags

    chunks = re.split('<article>([\S\s]+?</article>)', html)
    # lets remove the first and last chunk
    chunks.pop(0)
    if len(chunks) > 0:  # in case we didn't get anything don't pop last element
        chunks.pop()

    for chunk in chunks:

        #print(u"##### Got chunk: <{}>".format(chunk))
        title = ""
        url = ""
        ...
        if m:
            ...
            if options.debug:
                print("Failed to match a Vogue.in URL in chunk: <{}> source: <{}>".format(chunk, html_source))
